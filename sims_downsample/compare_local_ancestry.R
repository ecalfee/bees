...
d = do.call(rbind, lapply(ids, function(id) 
  read.table(paste0("../data/sims_downsample/ancestry_hmm/est_t2/", 
...
continents_long = c("AfrAfr", "AfrEu", "AfrEu", "EuEu", "EuEu", "EuEu")
...
dir_prefix = "../data/sims_downsample/ancestry_hmm/wSeqError/"
...
ACM_to_continent = function(posteriors){
  post_continent = mutate(posteriors, AfrAfr = X2.0.0) %>%
    mutate(., AfrEu = X1.1.0 + X1.0.1) %>%
    mutate(., EuEu = X0.2.0 + X0.1.1 + X0.0.2) %>%

...
# set up bootstrap -- for each original data point n, randomly resample a window, and then a snp within that window
# (this is equivalent to randomly sampling n snps, where they have weights = 1/k where k is # snps in their window)
# actually, this gives the incorrect perception of more independent parts (n) than their really are..
...
bootstrap_steep_clines <- function(windows_with_data, n_windows, n_boot, top_1perc, top_5perc, name = PREFIX){
  ...
  boot %>%
    ...
    summarise(top1_orig = sum(b > top_1perc), # original cutoff
              ...
              mean_b = mean(b),
              n = n()) %>%
    pivot_wider(data = ., names_from = r_bin5, values_from = c(top1_orig, top5_orig, top1_sample, top5_sample, mean_b, n)) %>%
    mutate(name = name, 
           n_boot = n_boot,
           top1_cutoff = top_1,
           top5_cutoff = top_5)

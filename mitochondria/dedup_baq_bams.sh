...
# note: all paths relative to bees/mitochondria/
ID=$1 # bee ID
DIR_IN="results/bam_files" # directory for input
BAM_SORTED=${DIR_IN}"/"${ID}".sort.bam" # full path to starting bam
DIR_OUT="results/bam_files" # results directory
DIR_METRICS="metrics" # metrics directory
BAM_OUT=${DIR_OUT}"/"${ID}".sort.dedup.baq.bam"
DIR_TMP=${DIR_OUT}"/tmp/"${ID} # for memory overflow
REF="../data/honeybee_genome/mitochondria/KY926884.fasta" # honeybee mtDNA reference genome (indexed by samtools faidx)

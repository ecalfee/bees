...
# spoetter qtls
spoetter <- read.table("../data/Spoetter_2012_QTLs/spoetter_2012_qtl_markers.csv",
                       sep = ",", header = T, stringsAsFactors = F) %>%
  filter(marker_type %in% c("start", "end") &
           QTL_study == "Spoetter 2012")
# 44k snp chip
...
filter(snps_44k, linkage.group == "Group6") %>% 
  arrange(Cooridnate) %>% tail() %>%
  filter(Cooridnate == 14575555)
filter(snps_44k, linkage.group == "Group16") %>% 
  arrange(Cooridnate) %>% tail() %>%
  filter(Cooridnate == 5626467)
filter(snps_44k, linkage.group == "Group16") %>% 
  arrange(Cooridnate) %>%
  filter(Cooridnate %in% c(3194214, 3198573)) %>%
  head()
filter(snps_44k, linkage.group == "Group16") %>% 
  arrange(Cooridnate) %>%
  filter(Cooridnate < 3194214) %>%
  tail()
filter(snps_44k, linkage.group == "Group16") %>% 
  arrange(Cooridnate) %>%
  filter(Cooridnate > 3194214) %>%
